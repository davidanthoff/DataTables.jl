module TestSort
    ...
    dv1 = DataValueArray(DataValue{Int}[9, 1, 8, NA, 3, 3, 7, NA])
    dv2 = DataValueArray(DataValue{Int}[9, 1, 8, NA, 3, 3, 7, NA])
    dv3 = DataValueArray(1:8)
    cv1 = DataValueCategoricalArray(dv1, ordered=true)

    d = DataTable(dv1 = dv1, dv2 = dv2, dv3 = dv3, cv1 = cv1)

    @test sortperm(d) == sortperm(dv1)
    @test sortperm(d[[:dv3, :dv1]]) == sortperm(dv3)
    @test isequal(sort(d, cols=:dv1)[:dv3], DataValueArray(sortperm(dv1)))
    @test isequal(sort(d, cols=:dv2)[:dv3], DataValueArray(sortperm(dv1)))
    @test isequal(sort(d, cols=:cv1)[:dv3], DataValueArray(sortperm(dv1)))
    @test isequal(sort(d, cols=[:dv1, :cv1])[:dv3], DataValueArray(sortperm(dv1)))
    @test isequal(sort(d, cols=[:dv1, :dv3])[:dv3], DataValueArray(sortperm(dv1)))
