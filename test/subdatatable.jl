module TestSubDataTable
    ...
    @testset "view -- DataTable" begin
        ...
        @test view(dt, DataValue{Int}[1, 2]) == head(dt, 2)
        @test view(dt, DataValue{Integer}[1, 2]) == head(dt, 2)
        @test view(dt, DataValue{UInt}[1, 2]) == head(dt, 2)
        @test view(dt, DataValue{BigInt}[1, 2]) == head(dt, 2)
        @test view(dt, DataValueArray([1, 2])) == head(dt, 2)
        @test view(dt, DataValueArray{Integer}([1, 2])) == head(dt, 2)
        @test view(dt, DataValueArray{UInt}([1, 2])) == head(dt, 2)
        @test view(dt, DataValueArray{BigInt}([1, 2])) == head(dt, 2)
        @test_throws NullException view(dt, [NA, 1])
    end
