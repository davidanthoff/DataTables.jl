module TestData
    ...
    #test_group("DataValueArray creation")
    nvint = DataValueArray(DataValue{Int}[1, 2, NA, 4])
    nvint2 = DataValueArray(5:8)
    ...
    dt4 = DataTable(Any[DataValueArray(1:4), DataValueArray(1:4)])
    dt5 = DataTable(Any[DataValueArray([1,2,3,4]), nvstr])
    ...
    #test_group("ref")
    @test isequal(dt6[2, 3], DataValue("two"))
    @test isnull(dt6[3, 3])
    @test isequal(dt6[2, :C], DataValue("two"))
    ...
    #test_group("assign")
    dt6[3] = DataValueArray(["un", "deux", "troix", "quatre"])
    @test isequal(dt6[1, 3], DataValue("un"))
    dt6[:B] = DataValueArray([4, 3, 2, 1])
    @test isequal(dt6[1,2], DataValue(4))
    dt6[:D] = DataValueArray([true, false, true, false])
    @test isequal(dt6[1,4], DataValue(true))
    delete!(dt6, :D)
    @test names(dt6) == [:A, :B, :C]
    @test size(dt6, 2) == 3
    ...
    #test_group("ref")
    @test isequal(sdt6a[1,2], DataValue(4))
    ...
    d1 = DataValueArray(rand(map(Int64, 1:2), N))
    d2 = DataValueCategoricalArray(DataValue{String}["A", "B", NA])[rand(map(Int64, 1:3), N)]
    d3 = DataValueArray(randn(N))
    d4 = DataValueArray(randn(N))
    dt7 = DataTable(Any[d1, d2, d3], [:d1, :d2, :d3])

    #test_group("groupby")
    gd = groupby(dt7, :d1)
    @test length(gd) == 2
    # @test isequal(gd[2]["d2"], CategoricalVector["A", "B", NA, "A", NA, NA, NA, NA])
    ...
    @test isequal(dt8[1:2, :d2], DataValueCategoricalArray(["A", "B"]))
    @test size(dt8, 1) == 3
    @test size(dt8, 2) == 5
    ...
    #test_group("reshape")
    d1 = DataTable(a = DataValueArray(repeat([1:3;], inner = [4])),
                   b = DataValueArray(repeat([1:4;], inner = [3])),
                   c = DataValueArray(randn(12)),
                   d = DataValueArray(randn(12)),
                   e = DataValueArray(map(string, 'a':'l')))
    ...
    d1s[:id] = DataValueArray([1:12; 1:12])
    d1s2[:id] =  DataValueArray([1:12; 1:12])
    ...
    srand(1)
    dt1 = DataTable(a = shuffle!(DataValueArray(1:10)),
                    b = DataValueArray(rand([:A,:B], 10)),
                    v1 = DataValueArray(randn(10)))

    dt2 = DataTable(a = shuffle!(DataValueArray(1:5)),
                    b2 = DataValueArray(rand([:A,:B,:C], 5)),
                    v2 = DataValueArray(randn(5)))
    ...
    m2 = join(dt1, dt2, on = :a, kind = :outer)
    # @test isequal(m2[:b2],
    #               DataValueArray(DataValue{String}["A", "B", "B", "B", "B",
    #                                              NA, NA,
    #                                              NA, NA, NA]))
    # @test isequal(m2[:b2],
    #               DataValueArray(DataValue{String}["B", "B", "B", "C", "B",
    #                                              NA, NA,
    #                                              NA, NA, NA]))

    dt1 = DataTable(a = DataValueArray([1, 2, 3]),
                    b = DataValueArray(["America", "Europe", "Africa"]))
    dt2 = DataTable(a = DataValueArray([1, 2, 4]),
                    c = DataValueArray(["New World", "Old World", "New World"]))

    m1 = join(dt1, dt2, on = :a, kind = :inner)
    @test isequal(m1[:a], DataValueArray([1, 2]))

    m2 = join(dt1, dt2, on = :a, kind = :left)
    @test isequal(m2[:a], DataValueArray([1, 2, 3]))

    m3 = join(dt1, dt2, on = :a, kind = :right)
    @test isequal(m3[:a], DataValueArray([1, 2, 4]))

    m4 = join(dt1, dt2, on = :a, kind = :outer)
    @test isequal(m4[:a], DataValueArray([1, 2, 3, 4]))

    # test with nulls (issue #185)
    dt1 = DataTable()
    dt1[:A] = DataValueArray(["a", "b", "a", NA])
    dt1[:B] = DataValueArray([1, 2, 1, 3])

    dt2 = DataTable()
    dt2[:A] = DataValueArray(["a", NA, "c"])
    dt2[:C] = DataValueArray([1, 2, 4])

    m1 = join(dt1, dt2, on = :A)
    @test size(m1) == (3,3)
    @test isequal(m1[:A], DataValueArray(["a","a", NA]))

    m2 = join(dt1, dt2, on = :A, kind = :outer)
    @test size(m2) == (5,3)
    @test isequal(m2[:A], DataValueArray(["a", "b", "a", NA, "c"]))

    srand(1)
    dt1 = DataTable(
        a = DataValueArray(rand([:x,:y], 10)),
        b = DataValueArray(rand([:A,:B], 10)),
        v1 = DataValueArray(randn(10))
    )

    dt2 = DataTable(
        a = DataValueArray([:x,:y][[1,2,1,1,2]]),
        b = DataValueArray([:A,:B,:C][[1,1,1,2,3]]),
        v2 = DataValueArray(randn(5))
    )
    dt2[1,:a] = NA
    ...
    # @test isequal(m1[:a], DataValueArray(["x", "x", "y", "y", fill("x", 5)]))
    # m2 = join(dt1, dt2, on = ["a","b"], kind = :outer)
    # @test isequal(m2[10,:v2], NA)
    # @test isequal(m2[:a],
    #               DataValueArray(DataValue{String}["x", "x", "y", "y",
    #                                              "x", "x", "x", "x", "x", "y",
    #                                              NA, "y"])
    ...
    dt1 = DataTable(a = DataValueArray(["a", "b", "a", "b", "a", "b"]),
                    b = DataValueArray(1:6),
                    c = DataValueArray([1:3;1:3]))
    dt = vcat(dt1, dt1)
